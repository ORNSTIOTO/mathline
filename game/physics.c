...
#define BOUNCE 0.1F
#define FF 0.2F // friction factor or smth
...
static void player_update(const float fdt)
{
	...
	Vector2 linear_acceleration = (Vector2){
		player->body.force.x / player->body.mass, 
	...
	if (player_collides(&coll)) {
		...
		if (slope.y>0) slope = Vector2Negate(slope);
		...
		for (int i = 0; i < 20; i++) {
			...
			if (!player_collides(&c)) break;
		}player->pos = Vector2Subtract(player->pos,  player->body.coll_nor  );
		...
		move = Vector2Reflect(player->body.linear_velocity, player->body.coll_nor);
		...
		float a = player->body.mass * PHYSICS_G * sinf(Vector2Angle(V2right, slope))
			- player->body.mass * PHYSICS_G * FF * cosf(Vector2Angle(V2right, slope));

		player->body.friction = Vector2Rotate((Vector2){a,0}, Vector2Angle(V2right, slope));
